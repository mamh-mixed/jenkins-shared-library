...
`deployJavaWeb` 的旧字段 `DEPLOY_PIPELINE.stepsBuildMaven` 在合并入口转换到当前结构 `DEPLOY_PIPELINE.stepsBuild.stepsBuildMaven`。转换发生在应用 Jenkins 参数之前，使 Jenkins 参数只需面向当前统一结构。

兼容规则采用“新结构优先、旧结构补缺”：

- 只有旧字段存在且当前 `stepsBuild.stepsBuildMaven` 缺失时，才用旧值补齐。
...
3. 清理上次遗留的 `.app-next` 和 `.app-previous`，重新创建 `.app-next`。
4. 根据 `archiveType` 将新包完整解压到 `.app-next`。
5. 校验 `.app-next` 至少包含一个文件或目录；空包视为失败。
6. 仅在前述步骤全部成功后执行目录切换：
   - 若线上 `app` 存在，将其重命名为 `.app-previous`。
   - 将 `.app-next` 重命名为 `app`。
   - 若第二次重命名失败，将 `.app-previous` 恢复为 `app`，然后返回失败。
   - 切换成功后删除 `.app-previous`。
7. 输出新 `app` 目录内容，保留现有部署可观察性。
