...
```json
{
  ...
  "pattern": [
    ...
    [
      {"k":"host","op":"eq","v":["jenkins.nanjingzhengshang.com"],"sub_k":""},
      {"k":"method","op":"eq","v":["POST"],"sub_k":""},
      {"k":"uri_no_query","op":"re","v":["^(?:/job/[^/]+)+/\\d+/replay/(?:checkScript|checkScriptCompile|run)$"],"sub_k":""}
    ]
  ]
}
```
...
Expected: `Strict-Transport-Security: max-age=31536000` 只出现一次；HTTPS `/login` 返回 200；HTTP 到 HTTPS 不循环重定向。
